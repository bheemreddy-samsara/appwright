...
Video recordings are downloaded automatically after each session and attached to reports.

#### IAM Role Assumption

To use an IAM role instead of static credentials, specify `roleArn` in the device config:

```ts
device: {
  provider: "aws-device-farm",
  projectArn: "arn:aws:devicefarm:us-west-2:123456789012:project:abc123",
  deviceArn: "arn:aws:devicefarm:us-west-2::device:Apple:iPhone.15:16.4.1",
  roleArn: "arn:aws:iam::123456789012:role/DeviceFarmAccess",
  roleSessionName: "ci-tests", // optional, defaults to "appwright-device-farm"
  externalId: "my-external-id", // optional, if required by the role's trust policy
},
```

Appwright will use STS AssumeRole to obtain temporary credentials before making Device Farm API calls.
