...
export class AWSDeviceFarmProvider implements DeviceProvider {
  ...
  private async createRemoteSession(): Promise<RemoteAccessSession> {
    const createSessionResponse = await this.client.send(
      new CreateRemoteAccessSessionCommand({
        ...
        interactionMode:
          this.deviceConfig.interactionMode ?? "VIDEO_ONLY",
        remoteRecordEnabled: this.deviceConfig.remoteRecordEnabled ?? true,
        name:
          this.deviceConfig.sessionName ??
          `${this.project.name}-${Date.now()}`,
  ...
  private buildCapabilities(
    ...
    if (this.deviceConfig.additionalCapabilities) {
      Object.assign(
        capabilities,
        this.deviceConfig.additionalCapabilities,
      );
    }
  ...
  static async downloadVideo(
    ...
    if (!sessionArn) {
      logger.warn("AWS Device Farm: session ARN missing, skipping video download.");
      return null;
    }
    ...
    try {
      const downloadResult = await retry(
        async () => {
          ...
          if (!artifact?.url) {
            throw new Error(
              "AWS Device Farm: Video artifact not ready yet.",
            );
          }
  ...
  private static async downloadFromPresignedUrl(
    ...
    if (!reader) {
      throw new Error("AWS Device Farm: Artifact response is not readable.");
    }
