...
interface WebDriverConnectionOptions {
  protocol: string;
  hostname: string;
  port: number;
  path: string;
...
export class AWSDeviceFarmProvider implements DeviceProvider {
  ...
  private client: DeviceFarmClient;
  ...
  constructor(
    ...
    this.client = new DeviceFarmClient({ region: this.region });
  ...
  async getDevice(): Promise<Device> {
    ...
    const connectionOptions: WebDriverConnectionOptions = {
  ...
  private async assumeRole(): Promise<void> {
    ...
    const sessionName =
      this.deviceConfig.roleSessionName ?? "appwright-device-farm";

    logger.log(`AWS Device Farm: Assuming IAM role ${roleArn}`);
    ...
    try {
      ...
      logger.log(`AWS Device Farm: Successfully assumed role ${roleArn}`);
    } finally {
      stsClient.destroy();
    }
  }
